...
async def monitor_loop(monitor, connections):
    ...
    # Determine update interval based on whether any backend uses CLI fallback
    uses_cli = getattr(monitor, 'uses_cli_fallback', False)
    update_interval = config.CLI_FALLBACK_INTERVAL if uses_cli else config.UPDATE_INTERVAL

    if uses_cli:
        logger.info(f"Using CLI fallback polling interval: {update_interval}s")
    else:
        logger.info(f"Using library polling interval: {update_interval}s")
